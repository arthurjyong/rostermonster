...
The algorithm used in this study was developed specifically for this project and comprises two major parts. The first part is designed to generate call rosters, while the second part completes the full roster (Figure 1). Initially, the algorithm requires an input file in the form of a Microsoft Excel document (Figure 2) to generate any specified number of on-call rosters for selection. Within the input Excel document, users must provide MO names, the duration of rostering, and various requests from the MOs on the first sheet (Figure 2A). To enhance rostering accuracy, a list of public holidays can be added on the second sheet (Figure 2B). For a fairer distribution of call duties among MOs, cumulative call points can be included based on departmental needs (Figure 2C). Additionally, any MO under a specific training track can be noted on the fourth sheet (Figure 2D), as certain duties may be reserved for these trainees.
...
The logical flow underlying the first part of the roster generation process, which focuses on handling the MO requests and initial roster setup, has been illustrated in Figure 3. Based on the input Excel document, the algorithm initially generates an empty roster using the MO names and the input dates, and integrates the list of public holidays for accurate scheduling. It then proceeds to address each date individually, striving to fulfill MO requests as comprehensively as possible. The requests considered by the algorithm include various types of leave (such as annual, medical, and training leave), call and no-call preferences, as well as preferences for working or not working on particular weekends. Following this, the algorithm reassesses each date to randomly assign any MOs who can mathematically be assigned, excluding those on leave or those with no-work requests for weekends. The assignment process also carefully factors in post-call conditions, where MOs who were on call the previous night, and "post-post-call" conditions, excluding MOs who had been on call two nights before, are not assigned to specific calls.

Generating even a single on-call roster manually can be a time-consuming task; however, the algorithm developed for this study can promptly generate any mathematically possible roster. For instance, when running on a 2018 entry-spec MacBook Pro, it takes approximately 1-2 seconds to produce a mathematically feasible call roster. While this demonstrates significant efficiency, these rosters are often less than ideal. For example, they might result in some MOs covering more call duties than others, indicating a potential area for further optimization. 
